  ...
  return (
    <section id="experience" className="py-16 md:py-24">
      ...
      <div className="relative max-w-3xl mx-auto">
        <div className="absolute left-1/2 -translate-x-1/2 h-full w-0.5 bg-gradient-to-b from-primary/50 via-accent/50 to-primary/50"></div>
        {experienceData.map((item, index) => (
          <div key={index} className="relative mb-12 flex items-center" style={{'justifyContent': index % 2 === 0 ? 'flex-start' : 'flex-end'}}>
            <div className="absolute left-1/2 -translate-x-1/2 bg-background border-2 border-primary rounded-full p-2 z-10">
              <Briefcase className="h-5 w-5 text-primary" />
            </div>
            <div
              className={`w-[calc(50%-2rem)] p-6 bg-card rounded-lg shadow-lg border-l-4 ${
                index % 2 === 0 ? "border-primary" : "border-accent text-right"
              }`}
            >
